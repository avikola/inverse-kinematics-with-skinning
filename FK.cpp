    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
	...
	for (int i = 0; i < jointUpdateOrder.size(); ++i)
	{
		...
		euler2Rotation(eulerAngles[jointUpdateOrder[i]], R_matrix, rotateOrders[jointUpdateOrder[i]]);
		R_M = Mat3d(R_matrix);

		// Orientation Angles:
		double R_o[9] = { 0,0,0,0,0,0,0,0,0 };
		euler2Rotation(jointOrientationEulerAngles[jointUpdateOrder[i]], R_o, RotateOrder::XYZ);
		R_O = Mat3d(R_o);
		
		local_T = R_O * R_M;

		localTransforms[jointUpdateOrder[i]] = RigidTransform4d(local_T, translations[jointUpdateOrder[i]]);
		...
		int order_j = jointUpdateOrder[i];
		if (jointParents[order_j] == -1)	// if root.
			globalTransforms[order_j] = localTransforms[order_j];
		else
			globalTransforms[order_j] = globalTransforms[jointParents[order_j]] * localTransforms[order_j];
	}
